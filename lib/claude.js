...
// Per-route overrides: diagnostic runs on Opus (deeper reasoning), SOAP stays
// on Sonnet (fast, structured generation). Fall back to MODEL if envs unset.
const DIAGNOSTIC_MODEL = process.env.ANTHROPIC_DIAGNOSTIC_MODEL || 'claude-opus-4-8';
const SOAP_MODEL = process.env.ANTHROPIC_SOAP_MODEL || 'claude-sonnet-4-6';
...
- Si un bloc « Suspicions diagnostiques déjà établies » est fourni dans le prompt utilisateur, reflète ces suspicions et leur hiérarchie dans la section « A — Analyse » (sans inventer d'autres hypothèses, sans recopier mot-à-mot les justifications fournies)
...
const CHAT_SYSTEM_BASE = `Tu es Menescau, un assistant IA expert en médecine vétérinaire. Tu aides les vétérinaires au quotidien. Tu peux :
 ...
 */
async function callClaudeJSON({ system, userPrompt, maxTokens = 2048, model = MODEL }) {
  const response = await client.messages.create({
    model,
    max_tokens: maxTokens,
    system: [
...
/**
 * Multimodal variant: accepts a list of files (images and/or PDFs) and a userText.
 * Each file is encoded as a Claude content block:
 *   - kind:'image' → { type:'image', source:{ type:'base64', media_type, data } }
 *   - kind:'pdf'   → { type:'document', source:{ type:'base64', media_type:'application/pdf', data } }
 * Base64 must NOT include the `data:...;base64,` prefix.
 */
async function callClaudeMultimodalJSON({ system, userText, files, maxTokens = 3000, model = MODEL }) {
  const blocks = [];
...
/** True for errors that may succeed on retry (rate limit, network, 5xx). */
...
module.exports = {
  ...
  ORDONNANCE_SYSTEM,
  // Prompt builders shared with other providers (OpenAI, Gemini)
  buildChatSystem,
  buildOrdonnancePrompt,
  callClaudeJSON,
  callClaudeImageJSON,
  callClaudeMultimodalJSON,
  callClaudeChat,
