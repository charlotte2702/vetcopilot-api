...
async function generateOrdonnancePdf(data) {
  ...
  return buildPdf(doc => {
    ...
    pdf.drawSignatureBlock(doc, { vet: vet || 'Dr. Laurent Dubreuil', ordreNum: ordreNum || '75-12345' });
  });
}
...
async function generateDiagnosticPdf(data) {
  ...
  return buildPdf(doc => {
    ...
    pdf.drawSignatureBlock(doc, { vet: vet || 'Dr. Laurent Dubreuil', ordreNum: '75-12345' });
  });
}
...
async function generateInvoicePdf(data) {
  ...
  return buildPdf(doc => {
    ...
    doc.restore();
    doc.y = badgeY + 24 + 20; // badge height + 20px gap to next section
    doc.x = MARGIN;
...
function drawInvoiceTable(doc, actes, total) {
  ...
  doc.y = totalY + totalH + 20; // 20px gap before next section
  doc.x = MARGIN;
}
