...
app.use(cors());
// Limit set to 32 MB so /api/diagnostic/image can accept up to ~20 MB cumulated
// of binary files (base64 expands by ~33%, so ~27 MB on the wire) with headroom.
// Anthropic caps total multimodal request at 32 MB; we stay below.
...
// Accepts an optional `files` array (images + PDFs, base64 inline) and an
// optional `vitals` object. When files are present we route through Claude's
// multimodal endpoint (same shape as /api/diagnostic/image) and skip the
// OpenAI fallback — gpt-4o-via-callOpenAIJSON is text-only here. Without
// files we keep the original text-only path with the GPT-4o safety net.
app.post('/api/diagnostic', async (req, res) => {
  const { animalId, symptoms, notes, vitals, files } = req.body || {};
  if (!symptoms && !notes && !(Array.isArray(files) && files.length)) {
    return res.status(400).json({ ok: false, error: 'champ "symptoms", "notes" ou "files" requis' });
  }

  // Validate attachments using the same caps as /api/diagnostic/image.
  let cleanFiles = [];
  if (Array.isArray(files) && files.length) {
    if (files.length > MAX_FILES) {
      return res.status(400).json({ ok: false, error: `trop de fichiers (${files.length}) — limite ${MAX_FILES}` });
    }
    let totalBytes = 0;
    for (let i = 0; i < files.length; i++) {
      const f = files[i] || {};
      const tag = f.name ? `"${f.name}"` : `#${i + 1}`;
      if (!f.base64 || typeof f.base64 !== 'string') {
        return res.status(400).json({ ok: false, error: `fichier ${tag} : "base64" requis` });
      }
      if (!f.mediaType || typeof f.mediaType !== 'string') {
        return res.status(400).json({ ok: false, error: `fichier ${tag} : "mediaType" requis` });
      }
      const isImg = ALLOWED_IMAGE_TYPES.has(f.mediaType);
      const isPdf = ALLOWED_DOC_TYPES.has(f.mediaType);
      if (!isImg && !isPdf) {
        return res.status(400).json({ ok: false, error: `fichier ${tag} : mediaType "${f.mediaType}" non supporté (autorisés : image/jpeg|png|gif|webp, application/pdf)` });
      }
      const cleanB64 = f.base64.replace(/^data:[^;]+;base64,/, '');
      const bytes = Buffer.byteLength(cleanB64, 'base64');
      if (bytes < 256) {
        return res.status(400).json({ ok: false, error: `fichier ${tag} : trop petit ou base64 invalide` });
      }
      if (bytes > MAX_FILE_BYTES) {
        const mb = Math.round((bytes / 1024 / 1024) * 10) / 10;
        return res.status(413).json({ ok: false, error: `fichier ${tag} : ${mb} MB > limite 5 MB/fichier` });
      }
      totalBytes += bytes;
      if (totalBytes > MAX_TOTAL_BYTES) {
        const mb = Math.round((totalBytes / 1024 / 1024) * 10) / 10;
        return res.status(413).json({ ok: false, error: `taille totale ${mb} MB > limite 20 MB cumulés` });
      }
      cleanFiles.push({ base64: cleanB64, mediaType: f.mediaType, kind: isPdf ? 'pdf' : 'image', bytes });
    }
  }
  ...
  const userPrompt = buildDiagnosticPrompt(animal, symptoms, notes, vitals, cleanFiles);

  // With attachments: Claude multimodal only (gpt-4o JSON helper here is text-only).
  if (cleanFiles.length) {
    try {
      const result = await claude.callClaudeMultimodalJSON({
        system: claude.DIAGNOSTIC_SYSTEM,
        userText: userPrompt,
        files: cleanFiles.map(f => ({ base64: f.base64, mediaType: f.mediaType, kind: f.kind })),
        maxTokens: 3000,
        model: claude.DIAGNOSTIC_MODEL,
      });
      return res.json({
        ok: true,
        input: {
          ...
          files: cleanFiles.map(f => ({ mediaType: f.mediaType, kind: f.kind, bytes: f.bytes })),
        },
        result: result.parsed,
        meta: { model: `anthropic/${claude.DIAGNOSTIC_MODEL}`, usage: result.usage },
      });
    } catch (err) {
  ...
  // Text-only path with GPT-4o fallback (unchanged behaviour).
  try {
    const { result, modelUsed } = await tryWithFallback({
      route: '/api/diagnostic',
      primaryFn: () => claude.callClaudeJSON({ system: claude.DIAGNOSTIC_SYSTEM, userPrompt, maxTokens: 3000, model: claude.DIAGNOSTIC_MODEL }),
      primaryLabel: `anthropic/${claude.DIAGNOSTIC_MODEL}`,
    ...
    res.json({
      ok: true,
      input: { animalId: animal?.id ?? null, symptoms: symptoms ?? '', notes: notes ?? '', vitals: vitals ?? null },
      result: result.parsed,
...
// --- Multimodal diagnostic (Claude vision + PDF) ------------------------
// Accepts a mix of images and PDFs in one shot. Routed exclusively through
// Claude (DIAGNOSTIC_MODEL) — Anthropic supports both image and document
// blocks natively, no OCR step needed. Per-file/total caps mirror Anthropic's
// 32 MB request ceiling: per-file ≤5 MB, cumulated ≤20 MB, ≤8 files.
...
app.post('/api/diagnostic/image', async (req, res) => {
  const { files, category, animalId } = req.body || {};

  if (!Array.isArray(files) || files.length === 0) {
    return res.status(400).json({ ok: false, error: 'champ "files" requis (tableau non vide d\'objets { base64, mediaType, kind })' });
  }
  if (files.length > MAX_FILES) {
    return res.status(400).json({ ok: false, error: `trop de fichiers (${files.length}) — limite ${MAX_FILES}` });
  }
  ...
  const animal = animalId != null ? ANIMALS.find(a => a.id === Number(animalId)) : null;
  const userText = buildImageDiagnosticPrompt(animal, category, cleanFiles);

  // 1 extra app-level retry on top of the SDK's own 2 retries — covers brief
  // network blips or post-quota recovery. Total = up to 3 attempts.
  const callOnce = () => claude.callClaudeMultimodalJSON({
  ...
  try {
    try {
      ...
      if (!claude.isTransientAnthropicError(err1)) throw err1;
      console.warn(`[/api/diagnostic/image] transient error, retrying once: ${err1.message}`);
      await new Promise(r => setTimeout(r, 700));
      try {
        result = await callOnce();
      } catch (err2) {
        if (claude.isTransientAnthropicError(err2)) {
          return res.status(503).json({ ok: false, error: 'Le service d\'analyse IA est momentanément indisponible. Réessaie dans quelques instants — ta consultation n\'est pas perdue.' });
        }
        throw err2;
      }
    }
...
function buildImageDiagnosticPrompt(animal, category, files) {
  const lines = [];
  if (animal) {
  ...
  if (Array.isArray(files) && files.length) {
    const imgCount = files.filter(f => f.kind !== 'pdf').length;
    const pdfCount = files.filter(f => f.kind === 'pdf').length;
    const parts = [];
    if (imgCount) parts.push(`${imgCount} image${imgCount > 1 ? 's' : ''}`);
    if (pdfCount) parts.push(`${pdfCount} PDF${pdfCount > 1 ? 's' : ''} (rapport, compte rendu d'analyse, etc.)`);
    lines.push(`Pièces jointes : ${parts.join(' + ')} — intègre-les toutes à ton analyse.`);
  }
  lines.push('');
  lines.push('Analyse les pièces jointes et fournis ton interprétation en JSON suivant la structure définie.');
  return lines.join('\n');
}

// --- SOAP (Claude Sonnet primary, GPT-4o fallback) ----------------------
app.post('/api/soap/generate', async (req, res) => {
  const { animalId, motif, examen, hypothese, traitement, diagResult } = req.body || {};
  bumpUsage('soap');
  const animal = animalId != null ? ANIMALS.find(a => a.id === Number(animalId)) : null;
  const userPrompt = buildSoapPrompt(animal, { motif, examen, hypothese, traitement, diagResult });

  try {
    const { result, modelUsed } = await tryWithFallback({
      route: '/api/soap/generate',
      primaryFn: () => claude.callClaudeJSON({ system: claude.SOAP_SYSTEM, userPrompt, maxTokens: 2500, model: claude.SOAP_MODEL }),
      primaryLabel: `anthropic/${claude.SOAP_MODEL}`,
...
function buildDiagnosticPrompt(animal, symptoms, notes, vitals, files) {
  const lines = [];
  if (animal) {
    lines.push(`Patient : ${animal.nm} — ${animal.sp} (${animal.breed}, ${animal.age}, ${animal.sex}, ${animal.wt})`);
  ...
  if (vitals && typeof vitals === 'object') {
    const v = [];
    if (vitals.temperature != null && vitals.temperature !== '') v.push(`T° ${vitals.temperature} °C`);
    if (vitals.heartRate != null && vitals.heartRate !== '') v.push(`FC ${vitals.heartRate} bpm`);
    if (vitals.respRate != null && vitals.respRate !== '') v.push(`FR ${vitals.respRate} /min`);
    if (vitals.mucMembranes) v.push(`Muqueuses : ${vitals.mucMembranes}`);
    if (vitals.generalState) v.push(`État général : ${vitals.generalState}`);
    if (v.length) lines.push(`Examen clinique : ${v.join(' · ')}`);
  }
  ...
  if (Array.isArray(files) && files.length) {
    ...
    const parts = [];
    if (imgCount) parts.push(`${imgCount} image${imgCount > 1 ? 's' : ''}`);
    if (pdfCount) parts.push(`${pdfCount} PDF${pdfCount > 1 ? 's' : ''} (bilan, compte rendu d'analyse, etc.)`);
    lines.push(`Pièces jointes d'examen : ${parts.join(' + ')} — intègre-les toutes à ton raisonnement diagnostique.`);
  }
  lines.push('');
...
function buildSoapPrompt(animal, { motif, examen, hypothese, traitement, diagResult }) {
  const lines = [];
  if (animal) {
  ...
  if (diagResult && Array.isArray(diagResult.hypotheses) && diagResult.hypotheses.length) {
    lines.push('');
    lines.push('--- Suspicions diagnostiques déjà établies par l\'assistant diagnostic ---');
    const urg = diagResult.urgencyLabel || diagResult.urgency;
    if (urg) lines.push(`Urgence évaluée : ${urg}`);
    diagResult.hypotheses.forEach((h, i) => {
      const prob = (h.prob != null) ? ` (${h.prob}%)` : '';
      const just = h.desc ? ` — ${h.desc}` : '';
      lines.push(`${i + 1}. ${h.nm}${prob}${just}`);
    });
    lines.push('Reflète ces suspicions et leur hiérarchie dans la section « A — Analyse » du SOAP, sans inventer d\'autres hypothèses et sans recopier mot-à-mot les justifications.');
  }
  lines.push('');
...
// --- Whisper transcription (universal voice fallback) ------------------
// Frontend POSTs { audio: base64, mime } when Web Speech API isn't available
// (Safari, Firefox, in-app browsers). Returns the transcript so the caller can
// feed it to the existing voice-entry / SOAP / chat flows.
const MAX_AUDIO_BYTES = 8 * 1024 * 1024; // 8 MB binary (~10 MB base64)
app.post('/api/transcribe', async (req, res) => {
  const { audio, mime } = req.body || {};
  if (!audio || typeof audio !== 'string') {
    return res.status(400).json({ ok: false, error: 'champ "audio" requis (base64)' });
  }
  bumpUsage('chat');
  try {
    ...
    if (buf.length > MAX_AUDIO_BYTES) {
      ...
      return res.status(413).json({ ok: false, error: `audio trop volumineux (${mb} MB) — limite 8 MB` });
    }
    if (buf.length < 1024) {
      return res.status(400).json({ ok: false, error: 'audio trop court ou invalide' });
    }
    const { text } = await openai.callOpenAIWhisper({ audioBuffer: buf, mime: mime || 'audio/webm' });
    res.json({ ok: true, text, modelUsed: 'openai/whisper-1' });
  } catch (err) {
    console.error('[/api/transcribe] failed:', err.message || err);
    const { status, message } = openai.mapOpenAIError(err);
    res.status(status).json({ ok: false, error: message });
  }
});
...
app.post('/api/voice-entry', async (req, res) => {
  ...
  const system = [
    "Tu es un assistant vétérinaire. Tu analyses un texte dicté par un vétérinaire et tu détectes ce qu'il veut faire : enregistrer un acte qui vient d'être réalisé (\"acte\"), planifier un rendez-vous futur (\"rdv\"), ou les deux dans la même phrase (\"both\"). Tu réponds en JSON STRICT, sans markdown, sans explication.",
    "",
    "Format attendu :",
    "{",
    '  "action": "acte" | "rdv" | "both",',
    '  "acte": null | {                                    // rempli si action = "acte" ou "both"',
    '    "animalName": string | null,',
    '    "actType": "vaccination"|"consultation"|"vermifuge"|"detartrage"|"controle"|"chirurgie"|"urgence"|"autre" | null,',
    '    "date": "YYYY-MM-DD" | null,                      // date de réalisation de l\'acte',
    '    "vaccineName": string | null,                     // si actType = vaccination',
    '    "weight": number | null,                          // poids en kg',
    '    "notes": string | null,                           // résumé clinique en 1-2 phrases',
    '    "nextVisit": "YYYY-MM-DD" | null,                 // rappel mentionné',
    '    "vetName": string | null',
    "  },",
    '  "rdv": null | {                                     // rempli si action = "rdv" ou "both"',
    '    "animalName": string | null,',
    '    "date": "YYYY-MM-DD" | null,                      // date du futur RDV',
    '    "time": "HH:MM" | null,                           // heure 24h ; \"matin\" → 09:00, \"après-midi\" → 14:00, \"fin d\'après-midi\" → 17:00',
    '    "type": "Consultation"|"Vaccination"|"Chirurgie"|"Urgence"|"Contrôle"|"Détartrage"|"Autre" | null,',
    '    "duration": number | null,                        // durée en minutes (15, 30, 45, 60)',
    '    "vetName": string | null,',
    '    "notes": string | null                            // motif détaillé éventuel',
    "  }",
    "}",
    "",
    `Date de référence (aujourd'hui) : ${todayISO}. Résous "aujourd'hui" / "hier" / "demain" / "mardi prochain" / "la semaine prochaine" / "dans 1 mois" / "le 15 juin" / "dans 10 jours" à partir de cette date.`,
    "",
    "Distinction clé :",
    "- \"acte\" : l'acte est PASSÉ (\"j'ai vu\", \"j'ai fait\", \"contrôle de…\", \"vaccination réalisée\").",
    "- \"rdv\" : c'est une demande de PLANIFICATION (\"prends rendez-vous\", \"programme\", \"RDV pour…\", \"planifie\", \"réserve un créneau\").",
    "- \"both\" : le véto rapporte un acte ET demande un suivi futur dans la même phrase (ex : \"j'ai vu Rex pour un vaccin CHPL et prends-lui un rendez-vous de contrôle dans un mois\").",
    "",
    "Pour les heures floues : \"matin\" = 09:00 · \"midi\" = 12:00 · \"début d'après-midi\" = 14:00 · \"après-midi\" = 15:00 · \"fin d'après-midi\" = 17:00 · \"soir\" = 18:00.",
    "Durée par défaut selon le type : Consultation 30 min · Vaccination 20 min · Chirurgie 60 min · Urgence 45 min · Contrôle 20 min · Détartrage 60 min.",
    "",
    ...
    animalList,
    "",
    "Vétérinaires : Dr. Patrick Hayot, Dr. Arnaud Vergnangeal, Dr. Axelle Freville, Dr. Miryea Barra, Dr. Séréna Vidé.",
    "",
    "Si action = \"acte\", remplis \"acte\" et mets \"rdv\": null. Si action = \"rdv\", remplis \"rdv\" et mets \"acte\": null. Si \"both\", remplis les deux. Les champs non mentionnés restent null. Ne devine pas. Réponds UNIQUEMENT avec l'objet JSON.",
  ].join('\n');
...
function renderPhotoUploadPage({ state, animalNm, token }) {
  ...
  const readyForm = state === 'ready' ? `
    <p class="lead">La clinique <strong>Menescau</strong> vous demande une photo de <strong>${animalNm}</strong> afin de l'ajouter à son dossier médical.</p>
...
<title>Menescau — Photo de ${animalNm}</title>
<style>
...
<div class="wrap">
  <header>
    ...
    <div class="brand">Menescau<small>Clinique Vétérinaire</small></div>
  </header>
  <div class="card">
