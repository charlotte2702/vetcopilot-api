...
const common = require('./templates/_common');
const ordonnanceTpl = require('./templates/ordonnance.html');
const diagnosticTpl = require('./templates/diagnostic.html');
const soapTpl = require('./templates/soap.html');
...
// --- PDF exports --------------------------------------------------------
// SOAP report → PDF
// SOAP report → PDF (Puppeteer-rendered HTML)
app.post('/api/soap/export-pdf', async (req, res) => {
  const { patient, soap, vet, ownerSummary, date, reportNum } = req.body || {};
  if (!soap || typeof soap !== 'object') {
    return res.status(400).json({ ok: false, error: 'champ "soap" requis (objet avec clés S, O, A, P)' });
  }
  const p = patient || {};
  const filename = `CR_${common.safeFilenamePart(p.nm || 'patient')}_${common.formatDateForFilename(new Date())}.pdf`;

  try {
    const html = soapTpl.render({ patient: p, soap, vet, ownerSummary, date, reportNum });
    const pdfBuf = await htmlToPdf(html, { footerTemplate: soapTpl.footerTemplate() });
...
// Diagnostic results → PDF (Puppeteer-rendered HTML)
app.post('/api/diagnostic/export-pdf', async (req, res) => {
  const { patient, symptoms, result, date, vet, ordonnanceNum } = req.body || {};
  ...
  const p = patient || {};
  const filename = `Diagnostic_${common.safeFilenamePart(p.nm || 'patient')}_${common.formatDateForFilename(new Date())}.pdf`;

  try {
    const html = diagnosticTpl.render({ patient: p, symptoms, result, date, vet, ordonnanceNum });
    const pdfBuf = await htmlToPdf(html, { footerTemplate: diagnosticTpl.footerTemplate() });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="${filename}"`);
    res.setHeader('Content-Length', pdfBuf.length);
    res.end(pdfBuf);
...
// Ordonnance → PDF (Puppeteer-rendered HTML)
app.post('/api/ordonnance/export-pdf', async (req, res) => {
  ...
  const filename = `Ordonnance_${common.safeFilenamePart(patient.nm)}_${common.formatDateForFilename(new Date())}.pdf`;

  try {
    const html = ordonnanceTpl.render({
      patient, clinique, pharmacie, alerteAllergies, diagnostic, ordonnanceNum, date, vet,
    });
    const pdfBuf = await htmlToPdf(html, { footerTemplate: ordonnanceTpl.footerTemplate() });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="${filename}"`);
    res.setHeader('Content-Length', pdfBuf.length);
    res.end(pdfBuf);
