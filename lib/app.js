...
// Try `primary` first; if it throws, log a warning and try `fallback`. Returns
// { result, modelUsed }. If both fail, throws an Error with both inner errors.
async function tryWithFallback(primaryFn, primaryLabel, fallbackFn, fallbackLabel) {
  try {
    const result = await primaryFn();
    return { result, modelUsed: primaryLabel };
  } catch (primaryErr) {
    const primaryMsg = (primaryErr && primaryErr.message) || String(primaryErr);
    console.warn(`[provider] ${primaryLabel} failed (${primaryMsg}) — falling back to ${fallbackLabel}`);
    try {
...
// --- Diagnostic (GPT-4o primary, Claude fallback) -----------------------
app.post('/api/diagnostic', async (req, res) => {
  ...
  try {
    const { result, modelUsed } = await tryWithFallback(
      () => openai.callOpenAIJSON({ system: claude.DIAGNOSTIC_SYSTEM, userPrompt, maxTokens: 3000 }),
      `openai/${openai.MODEL}`,
      () => claude.callClaudeJSON({ system: claude.DIAGNOSTIC_SYSTEM, userPrompt, maxTokens: 3000 }),
...
// --- SOAP (GPT-4o primary, Claude fallback) -----------------------------
app.post('/api/soap/generate', async (req, res) => {
  ...
  try {
    const { result, modelUsed } = await tryWithFallback(
      () => openai.callOpenAIJSON({ system: claude.SOAP_SYSTEM, userPrompt, maxTokens: 2500 }),
      `openai/${openai.MODEL}`,
      () => claude.callClaudeJSON({ system: claude.SOAP_SYSTEM, userPrompt, maxTokens: 2500 }),
      `anthropic/${claude.MODEL}`,
    );
    res.json({
      ok: true,
...
app.post('/api/ordonnance/generate', async (req, res) => {
  ...
  try {
    const { result, modelUsed } = await tryWithFallback(
      () => openai.callOpenAIJSON({ system: claude.ORDONNANCE_SYSTEM, userPrompt, maxTokens: 2500 }),
      `openai/${openai.MODEL}`,
      () => claude.callClaudeOrdonnance({ animal, diagnostic, symptoms }),
      `anthropic/${claude.MODEL}`,
    );
    res.json({
      ok: true,
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    const { result, modelUsed } = await tryWithFallback(
      () => gemini.callGeminiChat({ messages, system, maxTokens: 1500 }),
      `google/${gemini.MODEL}`,
      () => claude.callClaudeChat({ messages, animals: ANIMALS, appointments: APTS, animal }),
      `anthropic/${claude.CHAT_MODEL}`,
    );
