...
app.post('/api/ordonnance/export-pdf', (req, res) => {
  ...
  pdf.drawHeader(doc, {
    title: 'Ordonnance',
    subtitle: `N° ${num}${diagnostic ? '  ·  Diagnostic : ' + diagnostic : ''}`,
  ...
  if (clinique.length) {
    pdf.drawOrdonnanceSectionHeader(doc, { title: 'Délivré par la clinique (à remettre ce jour)', color: pdf.COLORS.TEAL });
    clinique.forEach((m, i) => pdf.drawMedicationRow(doc, m, i + 1, true));
  }

  if (pharmacie.length) {
    pdf.drawOrdonnanceSectionHeader(doc, { title: 'À acheter en pharmacie', color: pdf.COLORS.NAVY });
