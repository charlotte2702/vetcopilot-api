...
function createDoc() {
  return new PDFDocument({
    ...
    bufferPages: true,
    info: { Title: 'Menescau', Author: 'Menescau', Producer: 'Menescau' },
  });
}
...
// VetCopilot "V+" wordmark — navy rounded square + white V + gold cross.
// Drawn entirely with pdfkit primitives so it scales crisp at any size and
// requires zero external assets. Coordinates are in a 56-unit reference grid
// then scaled to the requested `size`.
function drawLogo(doc, x, y, size = 38) {
  const s = size / 56;
  const r = Math.round(12 * s); // corner radius
  doc.save();
  // Navy rounded background
  doc.roundedRect(x, y, size, size, r).fill(COLORS.NAVY);
  // White V (closed polygon: 6 vertices)
  doc.path(
    `M ${x + 12 * s} ${y + 14 * s} ` +
    `L ${x + 21 * s} ${y + 14 * s} ` +
    `L ${x + 28 * s} ${y + 36 * s} ` +
  ...
  // Gold "+" superscript (two crossed rounded rects, top-right area)
  doc.fillColor(COLORS.GOLD);
  doc.roundedRect(x + 41.5 * s, y + 9 * s,  3 * s, 13 * s, 0.5 * s).fill();
  doc.roundedRect(x + 36.5 * s, y + 14 * s, 13 * s,  3 * s, 0.5 * s).fill();
  doc.restore();
}
...
function drawHeader(doc, { title, subtitle, date }) {
  ...
  // V+ logo (38px) at left
  drawLogo(doc, MARGIN, 44, 38);

  // Clinic info next to logo
  doc.fillColor(COLORS.NAVY).font('Helvetica-Bold').fontSize(15).text('Menescau', MARGIN + 50, 46);
  doc.fillColor(COLORS.TXT2).font('Helvetica').fontSize(8.5)
    .text('Clinique Vétérinaire Berlioz', MARGIN + 50, 64)
    .text('6-8 rue Louise Ackermann, 06000 Nice  ·  04 93 87 39 42', MARGIN + 50, 75);
...
function drawOrdonnanceHeader(doc, { ordonnanceNum, diagnostic, date }) {
  ...
  const nameW = doc.widthOfString('Menescau');
  const groupW = logoSize + 12 + nameW;
  const groupX = (pw - groupW) / 2;
  const topY = 42;

  // V+ logo
  drawLogo(doc, groupX, topY, logoSize);

  // Clinic name right of logo, vertically centered
  doc.fillColor(COLORS.NAVY).font('Helvetica-Bold').fontSize(16).text('Menescau', groupX + logoSize + 12, topY + 9, { lineBreak: false });

  // Centered address line
  doc.fillColor(COLORS.TXT2).font('Helvetica').fontSize(8.5)
    .text('Clinique Vétérinaire Berlioz  ·  6-8 rue Louise Ackermann, 06000 Nice  ·  04 93 87 39 42',
...
//   Dr. Patrick Hayot                            ........... dotted line ...........
//   N° Ordre : 75-12345
//   Clinique Vétérinaire Berlioz
function drawSignatureBlock(doc, { vet, ordreNum }) {
  // ~70px content + 20px top breathing room → reserve 90 for the page-break check
  ensureSpace(doc, 90);
  const pw = doc.page.width;
  const y0 = doc.y; // disclaimer already left a 20px gap, no extra moveDown needed
  const innerW = pw - 2 * MARGIN;
  ...
  doc.fillColor(COLORS.TXT).font('Helvetica-Bold').fontSize(11)
    .text(safeText(vet || 'Dr. Patrick Hayot'), MARGIN, y0 + 12, { lineBreak: false });
  doc.fillColor(COLORS.TXT2).font('Helvetica').fontSize(8.5)
    .text(`N° Ordre des Vétérinaires : ${safeText(ordreNum || '75-12345')}`, MARGIN, y0 + 28, { lineBreak: false });
  doc.fillColor(COLORS.TXT2).font('Helvetica').fontSize(8.5)
    .text('Clinique Vétérinaire Berlioz', MARGIN, y0 + 41, { lineBreak: false });
...
// Measure-first disclaimer box: actually computes the wrapped body height
// at its real font/width before drawing the rectangle, so multi-line legal
// blurbs (e.g. the invoice's compliance paragraph) can never overflow into
// whatever section follows. Page-breaks if it doesn't fit.
function drawDisclaimer(doc, message) {
  const pw = doc.page.width;
  const w = pw - 2 * MARGIN;
  const PAD = 12;
  const GAP_TITLE = 6;
  const TRAILING = 20; // 20px space before next section
  const textW = w - 2 * PAD;
  const msg = safeText(message || '');

  // Measure each text block at its actual font/size/width
  doc.font('Helvetica-Bold').fontSize(8);
  const titleH = doc.heightOfString('ATTENTION', { width: textW, characterSpacing: 0.8 });
  doc.font('Helvetica').fontSize(8.5);
  const bodyH = doc.heightOfString(msg, { width: textW, lineGap: 1 });
  const boxH = PAD + titleH + GAP_TITLE + bodyH + PAD;

  ensureSpace(doc, boxH + TRAILING);
  const y = doc.y;

  // 1) Background at exact computed height
  doc.save();
  doc.roundedRect(MARGIN, y, w, boxH, 4).fill('#fdf6e3'); // pale gold
  doc.restore();

  // 2) Title — explicit positioning, no lineBreak (single line)
  doc.fillColor(COLORS.GOLD).font('Helvetica-Bold').fontSize(8)
    .text('ATTENTION', MARGIN + PAD, y + PAD, { width: textW, characterSpacing: 0.8, lineBreak: false });

  // 3) Body — wrapping enabled so nothing overflows
  doc.fillColor(COLORS.TXT2).font('Helvetica').fontSize(8.5)
    .text(msg, MARGIN + PAD, y + PAD + titleH + GAP_TITLE, { width: textW, lineGap: 1 });

  doc.y = y + boxH + TRAILING;
  doc.x = MARGIN;
}
...
function drawFooter(doc) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.fillColor(COLORS.MUTED).font('Helvetica').fontSize(7.5)
      .text("Généré par Menescau — L'IA est un outil d'aide, le vétérinaire garde la décision finale.",
