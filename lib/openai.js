 ...
 * Optional `model` overrides the default (e.g. to use `gpt-4o-mini`).
 */
async function callOpenAIJSON({ system, userPrompt, maxTokens = 2500, model = MODEL }) {
...
/**
 * Conversational call (no JSON mode). Accepts an Anthropic-style messages array
 * [{role: 'user'|'assistant', content}], rewrites it for OpenAI's chat API, and
 * returns { text, usage, stop_reason }. Used as the chatbot's fallback layer.
 */
async function callOpenAIChat({ system, messages, maxTokens = 1500, model = MODEL }) {
 ...
 */
async function callOpenAIImageJSON({ system, userText, imageBase64, mediaType, maxTokens = 3000, model = MODEL }) {
  const response = await client.chat.completions.create({
    model,
    max_tokens: maxTokens,
    response_format: { type: 'json_object' },
...
// Whisper transcription — universal fallback for browsers without Web Speech
// API (Safari, Firefox on some platforms, in-app browsers). Receives an audio
// buffer + mime, returns the transcribed text. Forces French to match the
// rest of the app's voice flows.
async function callOpenAIWhisper({ audioBuffer, mime = 'audio/webm', language = 'fr' }) {
  const ext = mime.includes('webm') ? 'webm'
            ...
            : 'webm';
  const file = await OpenAI.toFile(audioBuffer, `audio.${ext}`, { type: mime });
...
module.exports = { MODEL, callOpenAIJSON, callOpenAIChat, callOpenAIImageJSON, callOpenAIWhisper, mapOpenAIError };
